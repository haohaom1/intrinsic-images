...
class MikeNet(SuperModel):
    def __init__(self, input_size=(128, 128, 3)):
        ...
        # 1 by 1 conv layer
        x = Conv2D(3, (1, 1), activation='selu', padding='same')(input_img)
        ...
        x = Conv2D(256, (3, 3), activation='selu', padding='same')(x)
        x = MaxPooling2D((2, 2), padding='same')(x)
        x = Conv2D(512, (3, 3), activation='selu', padding='same')(x)
        x = Conv2D(512, (1, 1), activation='sigmoid', padding='same')(x)
        x = Conv2D(512, (1, 1), activation='sigmoid', padding='same')(x)
        encoded = MaxPooling2D((2, 2), padding='same')(x)
        # ensure everything is between 0 and 1

        # deconv structure for imap
        x = Conv2D(512, (3, 3), activation='selu', padding='same')(encoded)
        x = Conv2D(512, (3, 3), activation='selu', padding='same')(x)
        x = Conv2D(512, (3, 3), activation='selu', padding='same')(x)
