'''
    ...
    * Input: python create_training_1.py (kind: log/linear/srgb)
'''
...
def constrcut_labels(images, labels): 
    ...
    #Adds every label in labels to the dictionary and adds its relevant images. 
    for label in labels:
    ...
    #adds every image that doesn't have a label as a background label. 
    background = [] 
    for image in images: 
...
def add_matching_images(images_list, set_final, idx): 
    for img in images_list: 
        ...
        Y = [0, 0]
        Y[idx] = 1
        set_final.append([np.array(image), Y[:], img[1] + "_" + img[2]])
...
def create_list(SET, check):
    ...
    ttt_list = []
    for label in SET: 
        if label == "sweedish fish 1" or label == "sweedish fish 2" or label == "sweedish fish 3":
