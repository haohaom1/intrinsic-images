...
images_dir_2 = "../Dataset_Camera/Dataset_modified"
labels_dir = "../labels"
...
for i, image in enumerate(tqdm(images)):
	...
	if (image_name + ".xml" in labels):
		...
		if (name == "sweedish fish 1"):
			...
			if i in test_ids: 
				img = cv2.imread(os.path.join(images_dir_2, image), -1)
				img = cv2.resize(img, (IMG_SIZE, IMG_SIZE))
				test_data.append([np.array(img), [1, 0,0,0]])

				img = cv2.imread(os.path.join(images_dir, image), -1)
				img = cv2.resize(img, (IMG_SIZE, IMG_SIZE))
				test_data2.append([np.array(img), [1, 0,0,0]])


			else: 
				img = cv2.imread(os.path.join(images_dir, image), -1)
				img = cv2.resize(img, (IMG_SIZE, IMG_SIZE))
				train_data.append([np.array(img), [1, 0,0,0]])
			...
			if i in test_ids: 
				img = cv2.imread(os.path.join(images_dir_2, image), -1)
				img = cv2.resize(img, (IMG_SIZE, IMG_SIZE))
				test_data.append([np.array(img), [0, 1,0,0]])

				img = cv2.imread(os.path.join(images_dir, image), -1)
				img = cv2.resize(img, (IMG_SIZE, IMG_SIZE))
				test_data2.append([np.array(img), [0, 1,0,0]])

			else: 
				img = cv2.imread(os.path.join(images_dir, image), -1)
				img = cv2.resize(img, (IMG_SIZE, IMG_SIZE))
				train_data.append([np.array(img), [0, 1,0,0]])
			...
			if i in test_ids: 
				img = cv2.imread(os.path.join(images_dir_2, image), -1)
				img = cv2.resize(img, (IMG_SIZE, IMG_SIZE))
				train_data.append([np.array(img), [0, 0,1,0]])

				img = cv2.imread(os.path.join(images_dir, image), -1)
				img = cv2.resize(img, (IMG_SIZE, IMG_SIZE))
				test_data2.append([np.array(img), [0, 0,1,0]])
			...
			if i in test_ids: 
				img = cv2.imread(os.path.join(images_dir_2, image), -1)
				img = cv2.resize(img, (IMG_SIZE, IMG_SIZE))
				test_data.append([np.array(img), [0, 0,0,1]])

				img = cv2.imread(os.path.join(images_dir, image), -1)
				img = cv2.resize(img, (IMG_SIZE, IMG_SIZE))
				test_data2.append([np.array(img), [0, 0,0,1]])


			else: 
				img = cv2.imread(os.path.join(images_dir, image), -1)
				img = cv2.resize(img, (IMG_SIZE, IMG_SIZE))
				train_data.append([np.array(img), [0, 0,0,1]])
...
if (log_check == 1):
	...
	X_test2 = np.array([i[0].astype(float) for i in test_data2]).reshape(-1, IMG_SIZE, IMG_SIZE, 3)
	Y_test2 = np.array([i[1] for i in test_data2])
	...
	X_test2 = np.array([i[0] for i in test_data2]).reshape(-1, IMG_SIZE, IMG_SIZE, 3)
	Y_test2 = np.array([i[1] for i in test_data2])
